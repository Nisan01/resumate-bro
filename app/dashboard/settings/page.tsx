...
import AvatarSelector from "@/app/(auth)/_components/avatar";
...
function Modal({
  ...
  open: boolean; onClose: () => void; children: React.ReactNode; danger?: boolean;
}) {
  if (!open) return null;
  return (
    <div
  ...
  return (
    <button
      ...
      style={{
        ...
        border: "1px solid rgba(255,107,107,0.25)", fontFamily: "inherit",
      }}
    >
      {children}
    </button>
  ...
  return (
    <button
      ...
      style={{
        ...
        color: "#0a0714", fontFamily: "inherit",
      }}
    >
      {children}
    </button>
  ...
  return (
    ...
    >
      <div className="flex-1" style={{ minWidth: 0 }}>
        ...
        {desc && (
          <div className="text-xs leading-relaxed" style={{ color: "rgba(220,215,255,0.42)" }}>
            {desc}
          </div>
        )}
      </div>
...
function UsageStatCard({ icon, label, value, sub, accent = false }: {
  ...
  useEffect(() => {
    setDashOpacities(Array.from({ length: 8 }, () => 0.08 + Math.random() * 0.35));
  }, []);
      ...
      <div>
        <div
          ...
          style={{
            ...
            background: accent ? "linear-gradient(135deg,#7ee8fa,#c4b0ff)" : "linear-gradient(135deg,#c4b0ff,#ff9de2)",
            WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text",
          }}
        >
          {value}
        </div>
        <div className="mt-1.5 text-xs" style={{ color: "rgba(220,215,255,0.32)" }}>{sub}</div>
      </div>
      <div className="flex gap-1 mt-auto pt-1">
        {dashOpacities.length
          ? dashOpacities.map((o, i) => (
              <div key={i} className="rounded-full flex-1" style={{ height: 3, background: accent ? `rgba(126,232,250,${o})` : `rgba(196,176,255,${o})` }} />
            ))
...
export default function AccountPage() {
  ...
  const handleExportData = async () => {
    ...
    try {
      const response = await fetch("/api/auth/export-data", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId: user.id }) });
      if (!response.ok) throw new Error("Export failed");
      const contentType = response.headers.get("content-type") ?? "";
      if (contentType.includes("application/json")) {
        const res = await response.json();
        const safeName = (user?.name || user?.id || "user").replace(/\s+/g, "_").replace(/[^\w\-]/g, "");
  ...
  const handleAvatarSelect = async (url: string) => {
    ...
    try {
      const res = await fetch("/api/auth/update-avatar", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ avatarUrl: url, userId: user.id }) });
  ...
  const handlePasswordUpdate = async () => {
    ...
    try {
      const res = await fetch("/api/auth/update-pass", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ currentPassword, newPassword, userId: user.id }) });
      const data = await res.json();
      if (!data.success) throw new Error(data.message);
  ...
  const handleResetData = async () => {
    ...
    try {
      const response = await fetch("/api/reset-data", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId: user.id }) });
      if (!response.ok) {
  ...
  const handleDelete = async () => {
    if (!user?.id) { toast.error("User not found."); return; }
    if (deleteInput !== "DELETE") { setDeleteError(true); toast.error('You must type "DELETE" to confirm.'); return; }
    setDeleteError(false);
    setLoadingDelete(true);
    try {
      const response = await fetch("/api/auth/remove-account", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId: user.id }) });
      if (!response.ok) {
  ...
  const aiCost = calculatePrice(tokens.totalAllTokens);
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-6 sm:mb-8">
          <div
            ...
            style={{ background: "rgba(196,176,255,0.10)", border: "1px solid rgba(196,176,255,0.22)", color: "#c4b0ff" }}
          >
          ...
          <div>
            <h1
              ...
              style={{
                fontSize: "clamp(22px,4vw,34px)", letterSpacing: "-0.04em",
                background: "linear-gradient(135deg,#c4b0ff 0%,#7ee8fa 50%,#ff9de2 100%)",
                WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text",
              }}
            >
        ...
        <div className="flex flex-col gap-4">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5">
              ...
              <div className="flex gap-3 sm:gap-4 items-center">
                ...
                <div>
                  <div className="font-bold tracking-tight" style={{ fontSize: "clamp(18px,3vw,25px)", color: "#f0eeff", letterSpacing: "-0.03em" }}>
                    {user?.name ?? <Spinner className="size-6" />}
                  </div>
                  <div className="text-xs mt-0.5 truncate max-w-[200px] sm:max-w-none" style={{ color: "rgba(220,215,255,0.45)" }}>
                    {user?.email ?? "-"}
                  </div>
                  <div
                    className="inline-flex items-center gap-1.5 mt-1.5 rounded-full"
                    style={{ padding: "3px 10px", background: "rgba(196,176,255,0.10)", border: "1px solid rgba(196,176,255,0.20)", fontSize: 11, color: "#c4b0ff", fontWeight: 500 }}
                  >
              ...
              <div className="sm:text-right flex-shrink-0">
                ...
                <div className="font-extrabold leading-none" style={{ fontSize: "clamp(24px,4vw,32px)", color: "#f0eeff", letterSpacing: "-0.04em" }}>
                  FREE
                </div>
                <div className="text-xs mt-1" style={{ color: "rgba(220,215,255,0.35)" }}>forever</div>
                <div className="flex flex-col gap-1 mt-3 sm:items-end">
                  {["Unlimited generations", "Full AI analysis & ATS scoring", "Job Specific Practice Sessions", "Skills & Roadmap Guidance"].map((f) => (
                    <div key={f} className="flex items-center gap-1.5" style={{ fontSize: 11, color: "rgba(220,215,255,0.5)" }}>
          ...
          >
            <DataRow last title="Target Role & Industry" desc="Shown to the AI when generating resumes, scoring ATS, and building your roadmap.">
              <div className="flex flex-wrap items-center gap-2 mt-1 sm:mt-0">
                <span style={{ padding: "4px 10px", borderRadius: 999, fontSize: 11, fontWeight: 500, background: "rgba(196,176,255,0.10)", border: "1px solid rgba(196,176,255,0.22)", color: user?.targetRole ? "#c4b0ff" : "rgba(196,176,255,0.35)", whiteSpace: "nowrap" }}>
                  {user?.targetRole || "No role set"}
                </span>
                <span style={{ padding: "4px 10px", borderRadius: 999, fontSize: 11, fontWeight: 500, background: "rgba(126,232,250,0.08)", border: "1px solid rgba(126,232,250,0.18)", color: user?.targetIndustry ? "#7ee8fa" : "rgba(126,232,250,0.35)", whiteSpace: "nowrap" }}>
                  {user?.targetIndustry || "No industry set"}
                </span>
                <BtnGhost sm onClick={() => setTargetDialogOpen(true)}>
          ...
          >
            ...
            <div className="flex flex-col xs:flex-row gap-3">
              ...
              <UsageStatCard
                accent
                icon={<svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><path d="M12 8v4l3 3" /></svg>}
                label="AI Credits Used"
                value={loading ? <Spinner className="size-6" /> : `$${aiCost}`}
                sub={loading ? "calculating…" : `${(tokens.totalAllTokens / 1000).toFixed(1)}K tokens consumed`}
              />
            </div>
          </SectionCard>
          ...
          >
            <DataRow title="Profile Avatar" desc="Update your profile picture. Choose from our avatar gallery — shown in the dashboard and on exported resumes.">
              <BtnGhost sm onClick={() => setIsAvatarSelectorOpen(true)} disabled={loadingAvatar}>
                {loadingAvatar ? <Spinner /> : <svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>}
          ...
          >
            <DataRow danger title="Reset All Data" desc="Permanently delete all your resumes, analyses, projects, skills data, and templates. Your account stays active.">
            ...
            <DataRow danger last title="Delete Account" desc="Permanently delete your Resumate account, cancel your subscription, and erase all associated data. This cannot be undone.">
        ...
        <div className="flex flex-col gap-3 mb-5">
          {[
            ...
            <div key={label} className="flex flex-col gap-1.5">
              <label className="text-xs font-medium" style={{ color: "rgba(220,215,255,0.5)" }}>{label}</label>
              <input
                type="password" placeholder="••••••••" value={value}
                onChange={(e) => set(e.target.value)}
                className="w-full text-sm rounded-xl outline-none px-3.5 py-2.5"
                style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.085)", color: "#f0eeff", fontFamily: "inherit" }}
              />
            </div>
          ))}
        </div>
      ...
      <Modal open={!!modals.reset} onClose={() => closeModal("reset")} danger>
        ...
        <p className="text-xs leading-relaxed mb-6" style={{ color: "rgba(220,215,255,0.45)" }}>
          This will permanently delete all your resumes, analyses, projects, skills data, and templates.
      ...
      <Modal open={!!modals.delete} onClose={() => closeModal("delete")} danger>
        ...
        <p className="text-xs leading-relaxed mb-4" style={{ color: "rgba(220,215,255,0.45)" }}>
          This will permanently delete your account, all resumes, analyses, projects and cancel your subscription.{" "}
          <strong style={{ color: "#f0eeff" }}>No way to recover this data.</strong>
          <br /><br />
        ...
        <input
          ...
          style={{
            ...
            color: "#f0eeff", fontFamily: "inherit",
